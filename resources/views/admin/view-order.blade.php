                ...
                <div style="padding: 20px;">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <form action="{{ route('admin.orders.updateStatus', $order->id) }}" method="POST">
                            @csrf
                            <select name="status" onchange="this.form.submit()" class="status-select status-{{ strtolower($order->status) }}" style="width: 100%; padding: 10px;">
                                <option value="pending" {{ strtolower(trim($order->status)) == 'pending' ? 'selected' : '' }}>Pending</option>
