...
@section('content')
    ...
    <div class="content-card" style="margin-bottom: 24px;">
        ...
        <form action="{{ route('admin.testimonials.store') }}" method="POST" style="padding: 20px;">
            @csrf
            <div style="display: grid; grid-template-columns: 1fr 240px 180px 140px; gap: 15px;">
                ...
                <div class="form-group"><label>Assign to Product</label><select name="product_id" class="form-control"><option value="">Home Page (General Review)</option>@foreach($products as $product)<option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>@endforeach</select></div>
    ...
    <div class="content-card">
        ...
        <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Name</th><th>Product</th><th>Stars</th><th>Review</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            @forelse($testimonials as $testimonial)
                <tr><td><strong>{{ $testimonial->name }}</strong></td><td>{{ $testimonial->product ? $testimonial->product->name : 'Home Page (General)' }}</td><td style="color: #fbbf24;">{{ str_repeat('★', $testimonial->rating) }}</td><td>{{ $testimonial->review_text }}</td><td><span class="status-badge {{ $testimonial->is_active ? 'status-success' : 'status-danger' }}">{{ $testimonial->is_active ? 'Visible' : 'Hidden' }}</span></td><td>
            ...
            @empty
                <tr><td colspan="6" style="text-align:center; padding:35px;">No reviews added yet.</td></tr>
            @endforelse
        </tbody></table></div>
    </div>
@endsection
