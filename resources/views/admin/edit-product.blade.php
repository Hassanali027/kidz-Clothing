    ...
    <div class="content-card">
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="admin-form">
            ...
            <div class="form-group">
                ...
                <input type="number" name="review_count" class="form-control" min="0" value="{{ old('review_count', $product->review_count ?? 0) }}" placeholder="e.g. 26">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="price" class="form-control" placeholder="0.00" step="0.01" value="{{ old('price', $product->price) }}" required>
                </div>
                <div class="form-group">
                    <label>Sale Price (Rs.)</label>
                    <input type="number" name="sale_price" class="form-control" placeholder="0.00" step="0.01" value="{{ old('sale_price', $product->sale_price) }}">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="color" class="form-control" placeholder="e.g. Red, Blue, Multi-color" value="{{ old('color', $product->color) }}">
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="stock_quantity" class="form-control" placeholder="0" value="{{ old('stock_quantity', $product->stock_quantity) }}" required>
                </div>
                <div class="form-group">
                    <label>Status</label>
                ...
                <div class="checkbox-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="display_sections[]" value="featured_products" 
                            {{ in_array('featured_products', $product->display_sections ?? []) ? 'checked' : '' }}>
                    ...
                    <label class="checkbox-label">
                        <input type="checkbox" name="display_sections[]" value="new_arrivals"
                            {{ in_array('new_arrivals', $product->display_sections ?? []) ? 'checked' : '' }}>
                        <span class="checkbox-text">
                    ...
                    <label class="checkbox-label">
                        <input type="checkbox" name="display_sections[]" value="shop_by_category"
                            {{ in_array('shop_by_category', $product->display_sections ?? []) ? 'checked' : '' }}>
                        <span class="checkbox-text">
                    ...
                    <label class="checkbox-label">
                        <input type="checkbox" name="display_sections[]" value="shop_by_age"
                            {{ in_array('shop_by_age', $product->display_sections ?? []) ? 'checked' : '' }}>
                        <span class="checkbox-text">
                        ...
                        <div id="productList" class="product-list">
                            @php
                                $allProducts = \App\Models\Product::where('status', 'active')
                                    ...
                                    ->orderBy('name', 'asc')
                                    ->get();
                                $selectedRelated = $product->related_products ?? [];
                            @endphp
                            @forelse($allProducts as $prod)
                                <div class="product-item" data-id="{{ $prod->id }}" data-name="{{ $prod->name }}" data-category="{{ $prod->category }}" data-selected="{{ in_array($prod->id, $selectedRelated) ? 'true' : 'false' }}">
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                try {
                    ...
                    selectedProducts = preSelected.map(id => parseInt(id));
                } catch(e) {
                    selectedProducts = [];
                }
