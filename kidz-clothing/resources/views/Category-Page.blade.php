...
.cpd-range-input { -webkit-appearance:none; appearance:none; position:absolute; width:100%; height:3px; background:transparent; border-radius:4px; outline:none; pointer-events:none; top:50%; transform:translateY(-50%); z-index:2; }
...
/* Pagination */
.cp-pagination { display:flex; justify-content:center; gap:8px; margin-top:40px; }
        ...
        <div class="cpd-body">
            ...
            <div class="cpd-section">
                ...
                <div class="cpd-acc-panel" id="cpd-panel-size">
                    <div class="cpd-cb-grid">
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '2-4Y') ? 'checked' : '' }}><span class="cpd-cb-label">2-4Y <em>(23)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '4-6Y') ? 'checked' : '' }}><span class="cpd-cb-label">4-6Y <em>(18)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '6-8Y') ? 'checked' : '' }}><span class="cpd-cb-label">6-8Y <em>(15)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '8-10Y') ? 'checked' : '' }}><span class="cpd-cb-label">8-10Y <em>(12)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '10-12Y') ? 'checked' : '' }}><span class="cpd-cb-label">10-12Y <em>(9)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '12-14Y') ? 'checked' : '' }}><span class="cpd-cb-label">12-14Y <em>(7)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == 'Newborn') ? 'checked' : '' }}><span class="cpd-cb-label">Newborn <em>(11)</em></span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb" {{ (isset($activeSize) && $activeSize == '0-3M') ? 'checked' : '' }}><span class="cpd-cb-label">0-3M <em>(8)</em></span></label>
                    </div>
                </div>
            </div>
            ...
            <div class="cpd-section">
                ...
                <div class="cpd-acc-panel" id="cpd-panel-type">
                    <div class="cpd-cb-grid">
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Casual Shirts</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">T-Shirts</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Trousers</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Shorts</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Frocks</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Dresses</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Shalwar Kameez</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Jogger Sets</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Jackets</span></label>
                        <label class="cpd-cb-item"><input type="checkbox" class="cpd-cb"><span class="cpd-cb-label">Sweaters</span></label>
                    </div>
                </div>
            </div>
                ...
                <div class="cpd-acc-panel" id="cpd-panel-price">
                    ...
                    <div class="cpd-dual-slider">
                        ...
                        <input type="range" class="cpd-range-input" id="cpd-range-min"
                               min="0" max="10000" value="1900" step="100">
                        <input type="range" class="cpd-range-input" id="cpd-range-max"
                               min="0" max="10000" value="4900" step="100">
                    </div>
                    <!-- Price Inputs -->
                    <div class="cpd-price-row">
                        <div class="cpd-price-box">
                            <span class="cpd-rs-label">Rs</span>
                            <input type="number" class="cpd-price-input" id="cpd-price-min" value="1900">
                        </div>
                        <span class="cpd-price-dash">—</span>
                        <div class="cpd-price-box">
                            <span class="cpd-rs-label">Rs</span>
                            <input type="number" class="cpd-price-input" id="cpd-price-max" value="4900">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="cpd-section">
                ...
                <div class="cpd-acc-panel" id="cpd-panel-avail">
                    <div class="cpd-avail-wrap">
                        <button class="cpd-avail-btn cpd-avail-btn--active" id="cpd-avail-in">In Stock <em>(230)</em></button>
                        <button class="cpd-avail-btn" id="cpd-avail-out">Out of Stock <em>(90)</em></button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.querySelectorAll('.cp-sort-item').forEach(function (item) {
            item.addEventListener('click', function () {
                ...
                // Update button text (keep chevron svg)
                sortBtn.childNodes[0].textContent = item.textContent + ' ';
        ...
        if (rangeMin && rangeMax && track) {
            ...
            inputMin.addEventListener('input', function () {
                rangeMin.value = inputMin.value; updateSlider();
            });
            inputMax.addEventListener('input', function () {
                rangeMax.value = inputMax.value; updateSlider();
            });
            updateSlider(); // init
        }
        ...
        // ── Pagination active ──
        document.querySelectorAll('.cp-page-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                document.querySelectorAll('.cp-page-btn').forEach(function (b) { b.classList.remove('cp-page-btn--active'); });
                btn.classList.add('cp-page-btn--active');
            });
        });

        // ── Pre-select filters from URL ──
        window.addEventListener('load', function() {
            document.querySelectorAll('.cpd-cb:checked').forEach(function(cb) {
                // Trigger change event to populate tags
